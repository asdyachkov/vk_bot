...
class AdminAccessor(BaseAccessor):
    async def create_admin(
        self, email: str, password: str, vk_id: int
    ) -> Admin | None:
        ...
        password = str(
            hashlib.pbkdf2_hmac(
                "sha256",
                hashlib.pbkdf2_hmac(
                    "sha256", password.encode(encoding="UTF-8"), salt, 100000
                ),
                salt_local,
                100000,
            )
        ...
        query = (
            ...
            .values(email=email, password=password, vk_id=vk_id, salt=str(salt))
        )
        ...
        if added_id:
            return Admin(id=added_id[0], email=email, vk_id=vk_id)
        return None

    async def is_admin_exist(self, email: str, vk_id: int) -> bool:
        query = select(AdminModel).where(
            or_(AdminModel.vk_id == vk_id, AdminModel.email == email)
        )
        async with self.app.database._engine.connect() as connection:
    ...
    async def is_admin_by_email(self, email: str) -> Admin | None:
        ...
        user = user_.fetchone()
        if user:
            return Admin(id=user[0], email=user[1], vk_id=user[4])
        return None

    async def get_admin_auth(self, email: str, password: str) -> Admin | None:
        query = select(AdminModel).where(
            and_(AdminModel.email == email, AdminModel.password == password)
        )
        async with self.app.database._engine.connect() as connection:
            user_: CursorResult = await connection.execute(query)
        user = user_.fetchone()
